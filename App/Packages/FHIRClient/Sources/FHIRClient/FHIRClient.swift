...
public actor FHIRClient {
    ...
    ///
    /// 需要它是因為不能假設 server 支援時間過濾或排序。實測 Siming 曾經接受 `date`
    /// 卻完全不套用（後來修好了），而預設排序至今不是時間序——於是「近 24 小時超出
    /// 參考值」如果只取第一頁，抽到的那 100 筆可能完全不含異常值，卡片就會安靜地顯示 0。
    ///
    /// 那個 bug 修好之後這段仍然要留著：**下一個 server 不會有同樣的保證。**
    ///
