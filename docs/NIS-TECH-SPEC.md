...
> **實測結果（2026-09-04，2026-09-05 更新）**：S1、S2、S3、S5 已實際驗過，逐項結果與
> 後續修復記在 [`STATUS.md` §2.5](STATUS.md)。摘要：
>
> - **S2 可用**——conditional create 正常，seed 靠它做冪等。但條件必須落在該型別**真的有
>   索引**的參數上：拿沒索引的參數當條件，查詢回傳未過濾的結果、比對到不相干的資源，
>   於是 server 回「已存在」而**靜默不寫入，還回報成功**。實測 `PractitionerRole` 就是
>   這樣被吃掉兩筆。
> - **S1 的 `PractitionerRole` 原本缺席，現已補上**（Siming v1.1.0）。
> - **S3 的 `date` 原本在值帶時間時被靜默忽略，現已修復**（解析失敗改回 400）。
> - **S5 batch 仍不支援**——那一項就是 §0 第二個關鍵判斷的阻斷點。
>
> ⚠️ 上述修復目前只存在於 Siming 的工作目錄，**尚未 commit**。
>
> 這批實測改變了這張表的用法：**「檢查項通過」不等於「可以信任」。** 至少要分成
> 「有實作」「有索引」「一般查詢與 `_summary=count` 是否一致」三層來看——Siming 有 20 個
> store 的 count 路徑與搜尋路徑漂移，同一個 query string 兩種問法會得到不同答案。
